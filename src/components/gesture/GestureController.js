...
const INSTRUCTIONS = [
  { icon: "✨", action: "Cursor", desc: "Move your index finger to point — high sensitivity enabled", color: "text-indigo-400" },
  { icon: "👌", action: "Click", desc: "Quick pinch and release to click on elements", color: "text-indigo-400" },
  { icon: "↕️", action: "Drag Scroll", desc: "Pinch and drag up/down to move the page", color: "text-indigo-400" },
  { icon: "👋", action: "Wave/Shuffle", desc: "Wave hand fast left-right to shuffle skills elements", color: "text-indigo-400" },
];
...
export default function GestureController() {
  ...
  const { setGestureActive, setHeadRotation, setShuffleTrigger, active, setActive } = useGestureContext();
  ...
  useEffect(() => {
    ...
    return () => { document.documentElement.style.cursor = ""; document.body.style.cursor = ""; };
  ...
  useEffect(() => {
    if (!active) { if (cursorRef.current) cursorRef.current.style.opacity = "0"; return; }
    const updateCursor = () => {
  ...
  const doClick = useCallback((x, y) => {
    ...
    setTimeout(() => cursorRef.current?.classList.remove("gesture-click-flash"), 250);
  ...
  const drawSkeletons = useCallback((handResults, faceLm, canvas, video) => {
    ...
    if (faceLm) {
      ctx.lineWidth = 1; 
      ctx.shadowBlur = 5; ctx.shadowColor = "#818cf8";
      ...
      try {
        ...
        const [hL, fL] = await Promise.all([
          ...
          FaceLandmarker.createFromOptions(vision, { 
            ...
            runningMode: "VIDEO", outputFacialTransformationMatrixes: true, numFaces: 1 
          })
        ]);
  ...
  useEffect(() => {
    ...
    let animationId, isActive = true, stream = null;
    ...
    const start = async () => {
      ...
      try {
        ...
        const loop = () => {
          ...
          const now = Date.now();
          const handResults = landmarker.current.detectForVideo(videoRef.current, now);
          const faceResults = faceLandmarker.current.detectForVideo(videoRef.current, now);
            ...
            if (hLm) {
              ...
              if (!isCurrentlyPinching) {
                if (previousX.current !== null) {
                  ...
                  if (dt > 0) {
                    const speed = dx / dt;
                    if (Math.abs(speed) > 0.0035 && now - lastSwipeTime.current > 1500) {
              ...
              if (!isCurrentlyPinching && !isPinched.current) {
                targetX.current = (0.5 + (normalizedX - 0.5) * MOUSE_SENSITIVITY) * window.innerWidth;
                targetY.current = (0.5 + (normalizedY - 0.5) * MOUSE_SENSITIVITY) * window.innerHeight;
              }
              ...
              if (isCurrentlyPinching) {
                if (!isPinched.current) {
                  ...
                  if (!dragActive.current && Math.abs(currentY - initialPinchY.current) > 0.025) {
                  ...
                  if (dragActive.current) {
                    ...
                    window.scrollBy({ top: deltaY * window.innerHeight * DRAG_SCALE, behavior: "auto" });
                    lastPinchY.current = currentY;
                  }
                }
              } else {
                if (isPinched.current) {
                  if (!dragActive.current) doClick(smoothX.current, smoothY.current);
          ...
          if (cursorLabelRef.current) {
            cursorLabelRef.current.textContent = isPinched.current ? (dragActive.current ? "DRAG" : "HOLD") : "●";
          }
          animationId = requestAnimationFrame(loop);
        };
        loop();
  ...
  }, [active, isGesturing, doClick, drawSkeletons, setHeadRotation]);

  return (
    <>
      <AnimatePresence>{showInstructions && active && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[250] bg-black/70 backdrop-blur-xl flex items-center justify-center p-6" onClick={() => setShowInstructions(false)}>
          <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="max-w-2xl w-full bg-[#0a0a0c] border border-white/10 rounded-[40px] shadow-2xl p-12 text-center" onClick={e => e.stopPropagation()}>
            <span className="text-[10px] uppercase tracking-[0.5em] text-indigo-400 font-bold">GESTURES</span>
            <h2 className="text-4xl font-black text-white mt-4 mb-2">HAND CONTROLS</h2>
            <p className="text-gray-500 mb-12 text-sm uppercase tracking-wider">Navigate the website hands-free</p>
            <div className="grid grid-cols-2 gap-4 mb-12">
              {INSTRUCTIONS.map(i => (
                <div key={i.action} className="bg-white/5 p-6 rounded-3xl flex items-center gap-5 text-left border border-white/5 hover:bg-white/10 transition-all group">
                  <div className="text-3xl filter drop-shadow-[0_0_8px_rgba(129,140,248,0.5)]">{i.icon}</div>
                  <div>
                    <div className="font-bold text-white leading-tight mb-1 group-hover:text-indigo-300 transition-colors">{i.action}</div>
                    <div className="text-[11px] text-gray-500 leading-snug uppercase tracking-tight">{i.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            <button onClick={() => setShowInstructions(false)} className="w-[200px] py-4 bg-indigo-500 hover:bg-indigo-600 text-white font-bold rounded-2xl transition-all shadow-[0_0_20px_rgba(99,102,241,0.3)] uppercase text-[11px] tracking-widest active:scale-95">Got It</button>
          </motion.div>
        </motion.div>
      )}</AnimatePresence>
      <div ref={cursorRef} className="gesture-cursor" style={{ opacity: 0 }}><div className="gesture-cursor-ring !border-indigo-500" /><div className="gesture-cursor-dot !bg-indigo-500" /><span ref={cursorLabelRef} className="gesture-cursor-label !text-indigo-400">●</span></div>
      <div className="fixed bottom-6 right-6 z-[200]"><div className="bg-[#0a0a0a]/90 backdrop-blur-3xl rounded-[32px] p-5 border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col gap-4 min-w-[220px]">
          <div className="flex items-center justify-between px-1"><span className="text-[10px] font-black text-indigo-400 tracking-[0.3em] uppercase">Gestures</span><button onClick={() => setActive(!active)} className={`w-12 h-6 rounded-full transition-all relative ${active ? 'bg-indigo-500' : 'bg-white/10'}`}><div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${active ? 'left-7' : 'left-1'}`} /></button></div>
          <AnimatePresence>{active && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 130, opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="relative overflow-hidden rounded-2xl bg-black border border-white/10 group">
              <video ref={videoRef} autoPlay playsInline muted className="w-full h-full object-cover opacity-60 group-hover:opacity-80 transition-opacity" />
              ...
              <div className="absolute top-2 left-2 flex items-center gap-1.5 bg-black/60 backdrop-blur-md px-2.5 py-1.5 rounded-full border border-white/10">
                ...
                <span className="text-[8px] font-black text-white uppercase tracking-[0.1em]">Camera Feed</span>
              </div>
              {isLoading && (
